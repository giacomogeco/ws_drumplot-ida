...
%%%%%%%%%% SET PATHS %%%%%%%%%%%
global working_dir slh stations
% [working_dir,slh]=iad_setpath;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
working_dir='/home/item/Documents/MATLAB/ws_drumplot';
slh='/';
%%%%%%%%%% STATIONS & PROCESSING PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
stations=ws_read_ascii2cell([working_dir,slh,'conf_files',slh,net,slh,ConfFileName]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     load('/home/item/Documents/MATLAB/ws_drumplot/matfiles/FRU/2015/FRU_20151209/FRU_20151209_120000.mat')
    streamID='4370';
    filename=[namestz,'_',datestr(nownames(i15),'yyyymmdd_HHMM')];
    
    filename=[repmat(filename,[4 1]),repmat(streamID,[4 1]),['z2';'n2';'e2';'x2'],repmat('.gcf',[4 1])];
    
    try
        pathname='/home/item/scream/data/';
        pathname=[pathname,...
            namestz,slh,...
            datestr(nownames(i15),'yyyy'),slh, ...
            namestz,'_',datestr(nownames(i15),'yyyymmdd'),slh,...
            ];
%         no2/2016/no2_20161129/
        data=read_guralp(pathname,filename);
        
%         no2_20161222_00304370x2
%         load(file)
        nw=length(data.tt);
        time=data.tt;
...
%     figure,semilogx(FF,PWD,'k'),grid on,hold on 
    ...
    for i=1:length(chf);
        ...
        for ii15=1:length(t15)-1
            ...
            if ~isempty(iii)
                [p,f] = pwelch(trace(iii),length(trace(iii)),length(trace(iii))-1,NFFT,stations.smp(1));
                dB(ii15)=mean(10*log10(p(j))-PWD(j));
            else
                dB(ii15)=-100;
            end
            EF.(['ch',num2str(chfi(i))])=dB;
...
%     PPP=cat(1,PPP,PP);
    ...
    ws_noise2dB_insert(EF,stations)
